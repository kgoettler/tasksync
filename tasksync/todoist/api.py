...
class SyncToken:
    ...
    def update(self, token, timestamp=None):
        self.token = token
        self.timestamp = (
            timestamp if timestamp is not None else SyncToken.get_timestamp()
        )
        return self
...
class SyncTokenManager:
    ...
    def __init__(self, basedir=None):
        ...
        if exists(self.file):
            ...
            self.tokens = SyncTokenDict(
                **{
                    key: SyncToken("*", timestamp)
                    for key in SyncTokenDict.__required_keys__
                }
            )
    ...
    def get(self, resource_types=None) -> SyncToken:
        ...
        return SyncToken(
            ...
            timestamp=timestamp
            if sync_token != "*"
            else int(datetime.datetime.min.strftime("%s")),
        )
    ...
    def update(self, data, resource_types=None):
        ...
        for resource_type in resource_types:
            for elem in data.get(resource_type, []):
                ...
                if existing_elem := next(
                    (x for x in getattr(self, resource_type) if x["id"] == elem["id"]),
                    None,
                ):
    ...
    def wrapper(*args, **kwargs):
        ...
        data = func(
            ...
            **{
                ...
                if fsig.parameters[x].default is inspect._empty
            },
        )
...
class TodoistSyncAPI:
    ...
    def pull(self, sync_token=None, resource_types=None):
        ...
        res = requests.post(
            ...
            data={
                ...
                "resource_types": '["all"]'
                if resource_types is None
                else str(resource_types).replace("'", '"'),
            },
        )
    ...
    @staticmethod
    @add_optional_kwargs
    def complete_item(id_: str, date_completed: str | None = None) -> dict:
        """Complete a task
    ...
    @add_optional_kwargs
    def create_section(
        name: str, temp_id: str, project_id: str, section_order: int | None = None
    ) -> dict:
